class SuperpoweredGlue {
  ...
  constructor() {
    ...
    this.__exportsToWasm__.emscripten_notify_memory_growth = this.onMemoryGrowth.bind(
      this
    );

    this.__exportsToWasm__.__createClass__ = this.createClass.bind(this);
    this.__exportsToWasm__.__createStaticProperty__ = this.createStaticProperty.bind(
      this
    );
    this.__exportsToWasm__.__createStaticMethod__ = this.createStaticMethod.bind(
      this
    );
    this.__exportsToWasm__.__createConstructor__ = this.createConstructor.bind(
      this
    );
    this.__exportsToWasm__.__createDestructor__ = this.createDestructor.bind(
      this
    );
    ...
    this.__exportsToWasm__.__createClassConstant__ = this.createClassConstant.bind(
      this
    );
    ...
    switch (buffer.__type__) {
      case 1:
        buffer.array = new Uint8Array(
          ...
          buffer.pointer,
          buffer.length < 0
            ? buffer.__arraybuffer__.byteLength - buffer.pointer
            : buffer.length
        );
        break;
      case 2:
      ...
      case 3:
        buffer.array = new Uint16Array(
          ...
          buffer.pointer,
          buffer.length < 0
            ? (buffer.__arraybuffer__.byteLength - buffer.pointer) / 2
            : buffer.length
        );
        break;
      case 4:
      ...
      case 5:
        buffer.array = new Uint32Array(
          ...
          buffer.pointer,
          buffer.length < 0
            ? (buffer.__arraybuffer__.byteLength - buffer.pointer) / 4
            : buffer.length
        );
        break;
      case 6:
      ...
      case 7:
        buffer.array = new BigUint64Array(
          ...
          buffer.pointer,
          buffer.length < 0
            ? (buffer.__arraybuffer__.byteLength - buffer.pointer) / 8
            : buffer.length
        );
        break;
      case 8:
  ...
  invokeMethod() {
    ...
    for (let index = arguments.length - 1; index > 0; index--) {
      if (arguments[index].array != undefined)
        arguments[index] = arguments[index].array.byteOffset;
  ...
  createClass(classnamePointer, classnameLen, sizeofClass) {
    ...
    let WASM = class {
      constructor() {
        ...
        for (let property of meta.properties)
      ...
      destruct() {
        ...
        if (meta.hasDestructor)
          glue[meta.name + "::~" + meta.name](this.__pointer__);
    ...
    glue.__classUnderConstruction__ = glue.__classes__[classname] = glue[
      classname
    ] = WASM;
  ...
  createPropertyFromDescriptor(object, descriptor) {
    let buffer = this.createViewFromType(
      descriptor.viewType,
      object.__pointer__ + descriptor.offset,
  ...
  createStaticPropertyFromDescriptor(wasmClass, descriptor) {
    ...
    if (descriptor.viewLength > 1)
      Object.defineProperty(wasmClass, descriptor.name, {
        get: function () {
          return buffer.array;
        },
        set: function (value) {
  ...
  createStaticProperty(
    ...
    this.createStaticPropertyFromDescriptor(
      this.__classUnderConstruction__,
      descriptor
    );
  }
  ...
  createMethod(methodnamePointer, methodnameLen, returnPointerType) {
    ...
    let wasmMethodname =
      this.__classUnderConstruction__.__meta__.name + "::" + methodname;
  ...
  createStaticMethod(methodnamePointer, methodnameLen, returnPointerType) {
    ...
    this.__classUnderConstruction__[methodname] = this.invokeFunction.bind(
      this[wasmMethodname]
    );
  }
    ...
    if (!this[methodname]) {
      ...
      for (let namespace in this.__functionsWithNamespace__) {
        if (this.__functionsWithNamespace__[namespace][methodname]) {
          this[methodname] = this.__functionsWithNamespace__[namespace][
            methodname
          ];
  ...
  async loadFromArrayBuffer(wasmCode, afterWASMLoaded = null) {
    ...
    await WebAssembly.instantiate(wasmCode, {
      ...
      this.__memorygrowview__ = new Uint8Array(
        this.linearMemory,
        this.__memorygrowpointer__,
        16
      );
      this.__functionsWithNamespace__ = {};

      let outputBuffer = this.__functions__.__malloc__(1024);
      let stringview = new Uint8Array(
        this.linearMemory,
        this.__functions__.__malloc__(1024),
        1024
      );
      for (let f in this.__functions__) {
        if (f != "__demangle__") {
          let length = this.__functions__.__demangle__(
            this.toWASMString(f, stringview),
            outputBuffer
          );
          if (length > 0) {
            ...
            if (split.length == 2) {
              if (!this.__functionsWithNamespace__[split[0]])
                this.__functionsWithNamespace__[split[0]] = {};
              this.__functionsWithNamespace__[split[0]][
                split[1]
              ] = this.__functions__[f];
            }
  ...
  logMemory() {
    console.log(
      "WASM memory " +
        ...
        " stack, " +
        this.niceSize(
          this.linearMemory.byteLength - this.__functions__.__heapbase__()
        ) +
        " heap, " +
  ...
  updateMemoryViews() {
    for (let buffer of this.__views__)
      this.updateBuffer(buffer, this.linearMemory);
    this.__memorygrowview__ = new Uint8Array(
      this.linearMemory,
      this.__memorygrowpointer__,
...
if (typeof exports === "object" && typeof module === "object")
  module.exports = SuperpoweredGlue;
