...
async function load(url) {
  ...
  await fetch(url)
    ...
    .then((audiofileArrayBuffer) => {
      ...
      let audiofileInWASMHeap = Superpowered.arrayBufferToWASM(
        audiofileArrayBuffer
      );

      // Decode the entire file.
      let decodedAudio = Superpowered.Decoder.decodeToAudioInMemory(
        audiofileInWASMHeap,
      ...
      let arrayBuffer = Superpowered.moveWASMToArrayBuffer(
        decodedAudio,
        Superpowered.AudioInMemory.getSize(decodedAudio) * 4
      );
