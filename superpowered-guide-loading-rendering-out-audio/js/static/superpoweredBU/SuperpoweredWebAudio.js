...
class SuperpoweredWebAudio {
  constructor(minimumSamplerate, superpowered) {
    AudioWorkletHasBrokenModuleImplementation =
    ...
    if (
      ...
      AudioWorkletHasBrokenModuleImplementation = false;
    ...
    if (
      ...
      this.audioContext.audioWorklet.addModule(url).then(() => {
        class SuperpoweredNode extends AudioWorkletNode {
          ...
          sendMessageToAudioScope(message, transfer = []) {
      ...
      import(url).then((processorModule) => {
        ...
        node.processor = new processorModule.default(
          this.Superpowered,
        ...
        node.onaudioprocess = function (e) {
          ...
          node.processor.processAudio(
            node.inputBuffer,
            node.outputBuffer,
            node.inputBuffer.array.length / 2
          );
...
if (
  ...
  class SuperpoweredAudioWorkletProcessor {
    ...
    onMessageFromMainScope(message) {}
    sendMessageToMainScope(message) {
      if (!this.loader.onmessage({ data: message }))
...
if (typeof exports === "object" && typeof module === "object")
  ...
  define([], function () {
    return { SuperpoweredGlue, SuperpoweredWebAudio, SuperpoweredTrackLoader };
  });
else if (typeof exports === "object")
  exports["SuperpoweredModule"] = {
    SuperpoweredGlue,
    SuperpoweredWebAudio,
    SuperpoweredTrackLoader
  };
